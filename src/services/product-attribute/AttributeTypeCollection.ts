...
export class AttributeTypeCollection extends Collection<AttributeTypeType, 'id', AttributeType> {
	static readonly BASE_URI = '/api/attribute-type';
	...
	static async list(shopId: number) {
		...
		const response = await this.httpClient.get<AttributeTypeType[]>(this.BASE_URI);
	...
	static async listByProduct(productId: number, shopId: number) {
		this.httpClient.setHeaders({'x-shop-id': shopId});
		const response = await this.httpClient.get<AttributeTypeType[]>(`${this.BASE_URI}/product/${productId}`);
